/* Рендерер слайдов и навигация */
function md(s){
  if(!s) return '';
  return s
    .replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>')
    .replace(/\{\{(.+?)\}\}/g, function(m, sym){
      var cls = 'mega-sym';
      if(sym === '+') cls += ' plus';
...
function renderBlock(b){
  switch(b.t){
    case 'badge':
      return '<div class="badge badge-'+(b.color||'purple')+'">'+md(b.v)+'</div>';
    ...
    case 'boldline': return '<p><strong>'+md(b.v)+'</strong></p>';
    ...
    case 'list':
      return '<ul class="items'+(b.bullet===false?' no-dot':'')+'">'+b.v.map(i=>'<li>'+md(i)+'</li>').join('')+'</ul>';

    case 'checklist':
      return '<div class="checklist">'+b.v.map(i=>{
        var sub = i.sub ? '<ul class="items" style="margin-top:6px">'+i.sub.map(s=>'<li>'+md(s)+'</li>').join('')+'</ul>' : '';
        return '<div class="check-item"><div class="check-ico">✓</div><div class="check-text">'+md(i.text)+sub+'</div></div>';
      }).join('')+'</div>';

    case 'groups':
      return '<div>'+b.v.map(g=>{
        var sub = g.sub ? '<ul class="items" style="margin-top:4px;margin-bottom:10px">'+g.sub.map(s=>'<li>'+md(s)+'</li>').join('')+'</ul>' : '';
        return '<div class="group-block"><div class="group-lead">'+md(g.lead)+'</div>'+sub+'</div>';
      }).join('')+'</div>';

    case 'numbered':
      return '<div class="num-list">'+b.v.map((i,idx)=>'<div class="num-item"><div class="num-circle">'+(idx+1)+'</div><div class="num-text">'+md(i)+'</div></div>').join('')+'</div>';

    case 'quote':
      return '<div class="quote-block"><p>'+md(b.v)+'</p>'+(b.by?'<div class="by">— '+md(b.by)+'</div>':'')+'</div>';

    case 'result':
      return '<div class="result-block"><p>'+md(b.v)+'</p>'+(b.by?'<div class="by">'+md(b.by)+'</div>':'')+'</div>';

    case 'card':
      return '<div class="card-generic"><div class="label">'+md(b.label)+'</div><h3 style="margin-bottom:4px">'+md(b.title)+'</h3><p style="margin-bottom:0">'+md(b.v)+'</p></div>';

    case 'poll':
      return '<div class="tag-row">'+b.v.map(o=>'<span class="tag">'+md(o)+'</span>').join('')+'</div>';

    case 'segment':
      var bodyChips = b.body.split(/,\s*/).map(x=>'<span class="seg-chip body">'+md(x)+'</span>').join('');
      var psycheChips = b.psyche.split(/,\s*/).map(x=>'<span class="seg-chip psyche">'+md(x)+'</span>').join('');
      return '<div class="segment-card">'
        +'<div class="big-num">'+md(b.num)+'</div>'
        +'<div class="card-body">'
        +'<h3>'+md(b.name.replace(/^\d+\.\s*/,''))+'</h3>'
        +'<p>'+md(b.cause)+'</p>'
        ...
        +'</div>'
        +'<p class="seg-resolve">'+md(b.resolve)+'</p>'
        +'</div>'
      +'</div>';

    case 'research':
      return '<div class="research-card">'
        +'<div class="big-num" style="font-size:76px">'+md(b.num)+'</div>'
        +'<div class="card-body">'
        +'<div class="cite">'+md(b.cite)+'</div>'
        +'<h4>'+md(b.title)+'</h4>'
        +'<p class="finding">'+md(b.v)+'</p>'
        +'<div class="doi">'+md(b.doi)+'</div>'
        +'</div></div>';

    case 'testimonial':
      return '<div class="testimonial-card"><div class="star">'+md(b.tag)+'</div><div class="name">'+md(b.name)+'</div><div class="meta">'+md(b.meta)+'</div><div class="quote-block" style="margin:0"><p>'+md(b.v)+'</p></div></div>';

    case 'price':
      var feats = '<ul class="items">'+b.features.map(f=>'<li>'+md(f)+'</li>').join('')+'</ul>';
      var strikeRows = b.rows.filter(r=>!r.final);
      var finalRow = b.rows.filter(r=>r.final)[0];
      var strikeHtml = strikeRows.map(r=>'<div class="price-strike">'+md(r.label)+': <s>'+md(r.val)+'</s></div>').join('');
      var finalHtml = finalRow ? '<div class="price-hero"><span class="price-hero-label">'+md(finalRow.label)+'</span><span class="price-hero-val">'+md(finalRow.val)+'</span></div>' : '';
    ...
    case 'scientists':
      return '<div class="sci-row">'+b.v.map(function(s){
        ...
        return '<div class="sci-card"><div class="sci-avatar">'+imgHtml+'</div><div class="sci-name">'+s.name+'</div><div class="sci-years">'+s.years+'</div></div>';
      }).join('')+'</div>';

    case 'qcards':
      return '<div class="qcard-row">'+b.v.map((item,i)=>'<div class="qcard"><div class="qcard-left"><div class="qcard-num">'+(i+1)+'</div><div class="qcard-icon">'+item.icon+'</div></div><div class="qcard-text">'+md(item.q)+'</div></div>').join('')+'</div>';
...
function renderSlide(s, idx){
  ...
  var inner = '<span class="slide-num">'+String(idx+1).padStart(2,'0')+' / '+SLIDES.length+'</span>'
    +'<span class="section-tag">'+s.sec+'</span>'
    +'<div class="progress" id="progress-'+idx+'"></div>'
    +'<div class="scroll-content'+(s.center?'':' top')+'">'
    + s.blocks.map(renderBlock).join('')
    +'</div>';

  return '<div class="'+cls+'" data-slide="'+(idx+1)+'">'+inner+'</div>';
}
...
function goTo(n){
  ...
  var pct = ((current-1)/(total-1)*100).toFixed(1);
  var prog = slideEls[current-1].querySelector('.progress');
